...
import { canvasElement, canvasKey, copyWallpaper, downloadWallpaper, generateWallpaper, imageDataURL, imagesLoaded, isMobile, wallpaperHeight, wallpaperWidth, exportScale } from './logic';
...
<template>
  <div class="fixed inset-0 bg-gradient-to-br from-purple-50 via-violet-50 to-purple-100 overflow-auto">
    <!-- 主要内容 -->
    <div class="relative z-10 flex flex-col h-full">
      <!-- 标题区域 -->
      <div class="flex-grow-1 flex flex-col justify-center p-2 md:p-8 max-w-4xl mx-auto w-full">
        <div class="text-center mb-4 md:mb-8">
          <h1 class="text-2xl sm:text-3xl md:text-5xl font-bold text-purple-500 mb-2 tracking-wide drop-shadow-sm">
            遐蝶 壁纸生成器
          </h1>
        </div>
        ...
        <div v-if="imagesLoaded" class="flex flex-wrap justify-center gap-3">
          ...
          <button @click="generateWallpaper" :disabled="!imagesLoaded"
            ...
            :class="imagesLoaded
              ? 'bg-gradient-to-r from-purple-500 to-violet-500 text-white hover:from-purple-600 hover:to-violet-600 hover:shadow-xl hover:-translate-y-1'
          ...
          <button @click="showGenerationSettings = true"
            class="flex items-center gap-2 px-4 py-2 md:px-6 md:py-2.5 rounded-full font-semibold bg-gradient-to-r from-purple-200 to-violet-200 text-purple-600 hover:from-purple-300 hover:to-violet-300 hover:text-purple-700 transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-1">
            <span class="text-lg">🎨</span>
            生成设置
          </button>
        </div>
      ...
      <div ref="canvasContainer" class="mb-2 mt--6 md:mb-8 flex-grow-10 h-0 flex items-center justify-center p-4">
        <div :style="wrapperStyle"
          class="relative border-3 border-purple-300 rounded-2xl shadow-2xl shadow-purple-500/20 bg-gradient-to-br from-purple-50/80 to-violet-50/60 overflow-hidden backdrop-blur-sm transition-all duration-300 hover:scale-101 hover:shadow-3xl hover:shadow-purple-500/30">
          <canvas :key="canvasKey" :ref="setCanvasEl" :style="canvasStyle" class="rounded-xl cursor-pointer"
            @click="handleCanvasClick"></canvas>
          <div v-if="!imagesLoaded" absolute inset-0 flex items-center justify-center text-2xl text-purple-600>
            加载中...
          </div>
        </div>
      </div>
      ...
      <!-- 分辨率信息 -->
      <div v-if="imagesLoaded" class="text-center mb-4">
        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-white/30 backdrop-blur-sm rounded-full border border-purple-200/30 text-sm text-purple-600">
          <span class="text-base">📐</span>
          <span>画布: {{ wallpaperWidth }} × {{ wallpaperHeight }}</span>
          <span class="text-purple-400">|</span>
          <span>导出: {{ wallpaperWidth * exportScale }} × {{ wallpaperHeight * exportScale }} ({{ exportScale }}x)</span>
        </div>
      </div>
    </div>
      ...
      <Transition enter-active-class="transition-opacity duration-200 ease-out"
        ...
        <div v-if="showAuthorDialog"
          ...
          <Transition enter-active-class="transition-all duration-200 ease-out"
            ...
            <div v-if="showAuthorDialog"
              class="relative bg-gradient-to-br from-purple-50 via-violet-50 to-purple-100 rounded-3xl shadow-2xl p-8 max-w-md mx-4 border-2 border-purple-200/50 backdrop-blur-md"
              @click.stop>

              <!-- 可爱的装饰圆点 -->
              <div class="absolute top-4 left-4 w-3 h-3 bg-purple-300/40 rounded-full animate-pulse"></div>
              <div class="absolute top-6 right-8 w-2 h-2 bg-violet-400/50 rounded-full animate-bounce"
                style="animation-delay: 0.5s;"></div>
              <div class="absolute bottom-6 left-6 w-4 h-4 bg-purple-200/60 rounded-full animate-pulse"
                style="animation-delay: 1s;"></div>

              <!-- 关闭按钮 -->
              <button @click="hideAuthorDialog"
                class="absolute top-4 right-4 w-8 h-8 bg-purple-100 hover:bg-purple-200 rounded-full shadow-sm flex items-center justify-center text-purple-600 hover:text-purple-700 transition-all duration-200 hover:scale-110">
              ...
              <div class="text-center mb-6">
                ...
                <h3 class="text-xl font-bold text-purple-600 mb-1">关于这个小工具</h3>
                <div class="w-12 h-1 bg-gradient-to-r from-purple-300 to-violet-300 rounded-full mx-auto"></div>
              </div>

              <!-- 作者信息 -->
              <div class="space-y-4 text-center">
                <!-- 开发者信息 -->
                <div class="bg-white/70 rounded-2xl p-4 border border-purple-200/30 shadow-sm">
                  <div class="text-sm text-purple-500 mb-1">💻 原开发者</div>
                  <a href="https://x.com/@KermanX_" target="_blank"
                    class="text-purple-600 font-semibold hover:text-purple-700 transition-colors duration-200 hover:underline">
                    _Kerman
                  </a>
                </div>
                ...
                <!-- 修改者信息 -->
                <div class="bg-white/70 rounded-2xl p-4 border border-purple-200/30 shadow-sm">
                  <div class="text-sm text-purple-500 mb-1">🔧 修改者</div>
                ...
                <!-- 角色信息 -->
                <div class="bg-white/70 rounded-2xl p-4 border border-purple-200/30 shadow-sm">
                  <div class="text-sm text-purple-500 mb-1">📺 角色</div>
                  <a href="https://zh.moegirl.org.cn/%E9%81%90%E8%9D%B6" target="_blank"
                    class="text-purple-600 font-semibold hover:text-purple-700 transition-colors duration-200 hover:underline">
                    崩坏：星穹铁道@遐蝶
                  </a>
                </div>
                ...
                <!-- 项目仓库 -->
                <div class="bg-white/70 rounded-2xl p-4 border border-purple-200/30 shadow-sm">
                  <div class="text-sm text-purple-500 mb-1">🌐 项目地址</div>
                  <a href="https://github.com/Azusa-mikan/mingfeng-wallpaper/tree/castorice" target="_blank"
                    class="text-purple-600 font-semibold hover:text-purple-700 transition-colors duration-200 hover:underline break-all">
                ...
                <!-- 特别感谢-->
                <div class="bg-white/70 rounded-2xl p-4 border border-purple-200/30 shadow-sm">
                  <div class="text-sm text-purple-500 mb-1">🎉 特别感谢</div>
                  <div class="text-purple-600 font-semibold">
                    表情包原作者：<a href="https://space.bilibili.com/391634082" target="_blank"
                      class="text-purple-600 font-semibold hover:text-purple-700 transition-colors duration-200 hover:underline break-all">
                      小熊貓
                    </a>
                    <br>
                ...
                <!-- 分享按钮 -->
                <div class="pt-4">
                  <div class="text-sm text-purple-500 mb-3">✨ 分享给朋友们</div>
                  <div class="flex flex-wrap justify-center gap-2 max-w-xs mx-auto">
                    <button @click="shareToPlatform('twitter')"
                      class="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-purple-100 to-purple-200 text-purple-700 rounded-full hover:from-purple-200 hover:to-purple-300 hover:text-purple-800 transition-all duration-200 text-sm font-medium shadow-sm hover:shadow-md hover:-translate-y-0.5">
                      <span>🐦</span>
                      Twitter
                    </button>
                    <button @click="shareToPlatform('telegram')"
                      class="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-cyan-100 to-purple-200 text-cyan-700 rounded-full hover:from-cyan-200 hover:to-purple-300 hover:text-cyan-800 transition-all duration-200 text-sm font-medium shadow-sm hover:shadow-md hover:-translate-y-0.5">
                      <span>✈️</span>
                      Telegram
                    </button>
  ...
  <footer class="fixed bottom-2 left-2 z-10">
    <div class="text-xs text-purple-400/70 bg-white/20 backdrop-blur-sm rounded-lg px-2 py-1 border border-purple-200/20">
      <a :href="config.icp.url" target="_blank" class="hover:text-purple-500 transition-colors">
        {{ config.icp.number }}
      </a>
      | 内容为 AI 创作，请仔细甄别
    </div>
  ...
  <img fixed z-1000 right-0 md:right-19 bottom-0 w-12 md:w-20 hover:scale-y-90 transform-origin-right-bottom transition-all
    active:translate-y-2 md:active:translate-y-4 src="../castorice/10.png" @click="showAuthorDialog = !showAuthorDialog"
